...
class NetflixBulkChecker:
    ...
    def start(self, folder):
        ...
        if not files:
            print("No cookie files found in ZIP!")
            return
        self.stats['total'] = len(files)
        print(f"Starting check on {len(files)} cookie files...")
...
def start_cmd(message):
    ...
    # Combo mode can be added later
    bot.reply_to(message, "🚀 Netflix Cookie Checker Bot\n\nSelect mode below 👇", reply_markup=markup)

@bot.message_handler(func=lambda m: m.text == "🍪 Cookie Mode (ZIP)")
def cookie_mode(message):
    bot.reply_to(message, "📤 Send me the ZIP file containing your .txt cookie files")
...
def handle_zip(message):
    ...
    bot.reply_to(message, "✅ ZIP received. Downloading & extracting...")
